...
@section('content')
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 mb-2">Gestion des Interventions</h2>
            <p class="text-gray-600 text-lg">Planification et suivi des interventions de maintenance</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('interventions.create') }}" class="btn-primary flex items-center space-x-3 px-6 py-3">
    ...
    <div class="content-card p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    <input type="text" id="search" placeholder="Rechercher par véhicule, type, statut..." 
    ...
    <div class="flex justify-between items-center mb-4">
        <p class="text-sm text-gray-600">
            <span id="results-count">{{ $interventions->total() }}</span> résultat(s) trouvé(s)
        </p>
    </div>
    ...
    <div class="content-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="interventions-table-body">
                    @forelse($interventions as $intervention)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ ucfirst($intervention->type_intervention ?? 'N/A') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $intervention->date_debut ? $intervention->date_debut->format('d/m/Y') : 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $statusClasses = [
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $priorityClass }}">
                                    {{ ucfirst($intervention->priorite ?? 'N/A') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $intervention->cout ? number_format($intervention->cout, 2) . ' €' : 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button onclick="deleteIntervention({{ $intervention->id }})" 
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="text-gray-500">
                                    ...
                                    <p class="text-lg font-medium">Aucune intervention planifiée</p>
                                    <p class="text-sm">Commencez par planifier une nouvelle intervention</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
...
function deleteIntervention(id) {
    deleteInterventionId = id;
    document.getElementById('delete-modal').classList.remove('hidden');
}

function closeDeleteModal() {
    document.getElementById('delete-modal').classList.add('hidden');
    deleteInterventionId = null;
}
...
document.getElementById('confirm-delete').addEventListener('click', function() {
    if (deleteInterventionId) {
        ...
        form.method = 'POST';
        form.action = `/interventions/${deleteInterventionId}`;
