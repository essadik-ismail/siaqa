...
@section('title', 'Gestion des Visites Techniques')

@section('content')
    <!-- Header -->
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 mb-2">Gestion des Visites Techniques</h2>
            <p class="text-gray-600 text-lg">Planification et suivi des visites techniques du parc automobile</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('visites.create') }}" class="btn-primary flex items-center space-x-3 px-6 py-3">
    ...
    <div class="flex justify-between items-center mb-4">
        <p class="text-sm text-gray-600">
            <span id="results-count">{{ $visites->total() }}</span> résultat(s) trouvé(s)
        </p>
    </div>
    ...
    <div class="content-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="visites-table-body">
                    @forelse($visites as $visite)
                        <tr class="hover:bg-gray-50">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $visite->vehicule->marque->nom }} {{ $visite->vehicule->modele }}
                                        </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $visite->date_visite ? $visite->date_visite->format('d/m/Y') : 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ ucfirst($visite->type_visite ?? 'N/A') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $statusClasses = [
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button onclick="deleteVisite({{ $visite->id }})" 
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="text-gray-500">
                                    ...
                                    <p class="text-lg font-medium">Aucune visite technique planifiée</p>
                                    <p class="text-sm">Commencez par planifier une nouvelle visite</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            <div class="mt-3 text-center">
                ...
                <div class="mt-2 px-7 py-3">
                    <p class="text-sm text-gray-500">
                        Êtes-vous sûr de vouloir supprimer cette visite technique ? Cette action est irréversible.
                    </p>
                </div>
...
function applyFilters() {
    ...
    // Filter table rows
    const rows = document.querySelectorAll('#visites-table-body tr');
    let visibleCount = 0;
...
document.getElementById('confirm-delete').addEventListener('click', function() {
    if (deleteVisiteId) {
        ...
        form.method = 'POST';
        form.action = `/visites/${deleteVisiteId}`;
