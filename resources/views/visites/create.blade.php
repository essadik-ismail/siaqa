...
@section('content')
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 mb-2">Nouvelle Visite</h2>
            <p class="text-gray-600 text-lg">Planifier une nouvelle visite technique pour un véhicule</p>
        </div>
    ...
    <div class="content-card p-8">
        <form method="POST" action="{{ route('visites.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="date_visite" class="block text-sm font-medium text-gray-700 mb-3">Date de visite *</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="type_visite" class="block text-sm font-medium text-gray-700 mb-3">Type de visite *</label>
                    <select name="type_visite" id="type_visite" required class="form-input w-full px-4 py-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="centre_visite" class="block text-sm font-medium text-gray-700 mb-3">Centre de visite</label>
                    <input type="text" name="centre_visite" id="centre_visite" value="{{ old('centre_visite') }}"
                           class="form-input w-full px-4 py-3" placeholder="Nom du centre">
                    @error('centre_visite')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="inspecteur" class="block text-sm font-medium text-gray-700 mb-3">Inspecteur</label>
                    <input type="text" name="inspecteur" id="inspecteur" value="{{ old('inspecteur') }}"
                           class="form-input w-full px-4 py-3" placeholder="Nom de l'inspecteur">
                    @error('inspecteur')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="cout" class="block text-sm font-medium text-gray-700 mb-3">Coût (€)</label>
                ...
                <div>
                    <label for="resultat" class="block text-sm font-medium text-gray-700 mb-3">Résultat</label>
                    <select name="resultat" id="resultat" class="form-input w-full px-4 py-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="prochaine_visite" class="block text-sm font-medium text-gray-700 mb-3">Prochaine visite</label>
                ...
                <div>
                    <label for="kilometrage_visite" class="block text-sm font-medium text-gray-700 mb-3">Kilométrage lors de la visite</label>
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="4" 
                          class="form-input w-full px-4 py-3" placeholder="Observations, remarques, points à vérifier...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex space-x-4 pt-6">
                <button type="submit" class="btn-primary flex-1 px-8 py-4 text-lg">
                    <i class="fas fa-save mr-3"></i>Planifier la visite
                </button>
                <a href="{{ route('visites.index') }}" class="flex-1 bg-gray-500 hover:bg-gray-600 text-white px-8 py-4 rounded-xl font-medium text-center transition-all duration-200 hover:scale-105">
                    Annuler
                </a>
            </div>
        </form>
