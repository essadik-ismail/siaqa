...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Nouvelle Charge</h1>
                <p class="text-gray-600">Ajouter une nouvelle charge à votre système</p>
            </div>
            ...
            <form method="POST" action="{{ route('charges.store') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="designation" name="designation" required
                            ...
                            @foreach($types as $key => $type)
                                <option value="{{ $key }}" {{ old('designation') == $key ? 'selected' : '' }}>{{ $type }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" id="montant" name="montant" value="{{ old('montant') }}" required min="0" step="0.01"
                    ...
                    <div>
                        ...
                        <input type="text" id="fichier" name="fichier" value="{{ old('fichier') }}"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror" placeholder="Description détaillée de la charge...">{{ old('description') }}</textarea>
                ...
                <div class="flex space-x-3 pt-6">
                    <button type="submit" class="flex-1 bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200">
                        <i class="fas fa-save mr-2"></i>Créer la charge
                    </button>
